# Adidas US Sales Analysis Dashboard

## Project Overview
This project analyses Adidas US sales data to understand sales performance across regions, products, retailers, and sales channels. The dashboard identifies key revenue drivers and highlights opportunities to improve business performance.

## Business Problem
Adidas wants to understand which products, regions, retailers, and sales channels generate the highest revenue and profitability in order to optimise marketing strategies and sales performance.

## Dataset
The dataset contains Adidas US sales transactions from January 2020 to July 2021.

Key variables include:
- Region
- State
- Retailer
- Product Category
- Sales Method (In-store, Online, Outlet)
- Units Sold
- Total Sales
- Operating Profit
- Invoice Date

## Tools Used
- Power BI
- Excel
- Data Visualization
- Business Analysis

## Dashboard Features

### Executive Overview
- Total Sales: $899.9M
- Average Sales: $93.27K
- Units Sold: ~2M

### Sales Performance Analysis
- Sales by Region
- Sales by Product
- Sales by Sales Method
- Monthly Sales Trends

### Product Performance
- Product category sales analysis
- Operating profit by product

### Advanced Sales Insights
- Trend volatility analysis
- Sales anomaly detection
- Product comparison insights

### Sales Drivers & Regional Insights
- Key Influencer analysis
- Sales distribution by state
- Retailer contribution analysis

## Key Insights
- Women's Street Footwear contributes **32.46% of product sales**
- Northeast region generates the **highest revenue (~$186M)**
- West Gear and Foot Locker are the **top contributing retailers**
- Online sales account for **32.85% of total revenue**
- Men's Apparel records the **lowest sales performance**

## Strategic Recommendations
- Prioritise marketing for high-performing categories such as Women's Footwear
- Improve performance of Men's Apparel through promotions and pricing strategies
- Expand online sales channels due to growing digital demand
- Strengthen partnerships with high-performing retailers

## Dashboard Preview
<img width="749" height="564" alt="Sales_Analysis 2" src="https://github.com/user-attachments/assets/0f1a0a37-3f59-45f7-b759-dcdf61e60718" />
<img width="1131" height="597" alt="Sales Performance Analysis" src="https://github.com/user-attachments/assets/e24644a8-7764-4123-8c54-631fe2ccab4e" />
<img width="615" height="554" alt="PRODUCT_DETAILS" src="https://github.com/user-attachments/assets/7ad959dd-6421-4231-b0a8-d5ecc8e1762a" />
<img width="1073" height="581" alt="Executive Sales Overview" src="https://github.com/user-attachments/assets/846a7bc4-d48c-4462-aac7-36e661a0a3c6" />

### Executive Sales Overview
![Executive Sales Overview]<img width="1073" height="581" alt="Executive Sales Overview" src="https://github.com/user-attachments/assets/846a7bc4-d48c-4462-aac7-36e661a0a3c6" />

### Sales Analysis 2
![Sales Analysis 2] <img width="749" height="564" alt="Sales_Analysis 2" src="https://github.com/user-attachments/assets/0f1a0a37-3f59-45f7-b759-dcdf61e60718" />

### Sales Performance Analysis
![Sales Performance Analysis] <img width="1131" height="597" alt="Sales Performance Analysis" src="https://github.com/user-attachments/assets/e24644a8-7764-4123-8c54-631fe2ccab4e" />

### Product Details
![Product Details] <img width="615" height="554" alt="PRODUCT_DETAILS" src="https://github.com/user-attachments/assets/7ad959dd-6421-4231-b0a8-d5ecc8e1762a" />

## Author
Aritri Mukherjee  
MSc Business Analytics & Finance – University of Southampton
